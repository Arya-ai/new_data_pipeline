	...
	def render_POST(self, request):
		...
		if img is not None:
			...
			if img.shape[0] > 256 or img.shape[1] > 256:
				...
				reactor.callInThread(predict(self.model, images, request))
			else:
				reactor.callInThread(predict(self.model, img, request))
		...
		'''
		RUN THE ABOVE CODE IN THE FUNCTION CALLED AND RETURN THE RESPONSE
		'''
		return NOT_DONE_YET
