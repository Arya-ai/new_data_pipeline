...
import random   # for shuffling the examples
import cPickle as pickle

root_dir = os.path.abspath('models/VGG19_12/')

random.seed(2017)   # for reproducibility
...
class Serialize():
		...
		def deserialize(self, options):
			...
			self.nInputPerRecord = int(options['nInputPerRecord'])
			self.nOutputPerRecord = int(options['nOutputPerRecord'])
			self.nb_epochs = int(options['epochs'])

			if 'train_val_split' in options:
				self.split = float(options['train_val_split'])
				logger.info("Train Validation Split: {}".format(self.split))
			else: self.split = 0.9

			self.inputDBs = []
			self.outputDBs = []

			for idx in range(self.nInputPerRecord):
				dbname = 'datumdb' + str(idx)
				self.inputDBs.append(dbname)

			for idx in range(self.nOutputPerRecord):
				dbname = 'labeldb' + str(idx)
				self.outputDBs.append(dbname)

			logger.debug("inputDBs: " + repr(self.inputDBs))
			logger.debug("outputDBs: "+ repr(self.outputDBs))
			...
			with self.env.begin(write=False, db=inputDBHandles[0]) as txn_named:
				self.nb_samples = txn_named.stat()['entries']
				logger.info("Number of samples: " + repr(self.nb_samples))

			# set number of batches
			self.nb_batches = self.nb_samples // self.batch_size

			keys = range(1, self.nb_samples + 1)
			self.nb_test_samples = int(self.nb_samples * 0.15)		# train_test_split = 85:15
			test_keys = keys[:self.nb_test_samples]
			# dump the test lmdb keys
			logger.info("Number of test samples: {}".format(self.nb_test_samples))
			try:
				test_keys_path = root_dir + 'test_keys.pkl'
				pickle.dump(test_keys, open(test_keys_path, 'w'))
			except Exception as e:
				logger.error("Cannot dump test keys: " + repr(e))

			keys = keys[self.nb_test_samples:]
			random.shuffle(keys)
			self.nb_train_samples = self.nb_samples - self.nb_test_samples
			pivot = int(self.split * self.nb_train_samples)
			train_keys = keys[:pivot]
			validation_keys = keys[pivot:]

			self.nb_train_samples = len(train_keys)
			self.nb_validation_samples = len(validation_keys)
			...
			try:
				assert self.nb_train_samples - pivot == self.nb_validation_samples
			except AssertionError as e:
				logger.error("AssertionError: " + repr(e))
			...
			self.input_shapes, self.output_shapes = self._get_datum_shapes(input_cursors, output_cursors)
			output_dists = self._get_data_distribution(output_cursors)

			return_dict = {'input_shapes': self.input_shapes, 'output_shapes': self.output_shapes, 'data_dists': output_dists,
							'train_generator': self.train_generator(input_cursors, output_cursors, train_keys),
							'validation_generator': self.validation_generator(input_cursors, output_cursors, validation_keys),
							'nb_train_samples': self.nb_train_samples, 'nb_validation_samples': self.nb_validation_samples,
							'nb_samples': self.nb_samples, 'batch_size': self.batch_size, 'epochs': self.nb_epochs}
		...
		def _get_data_distribution(self, output_cursors):
			output_dists = []
			for idx, output_cursor in enumerate(output_cursors):
				output_dist = np.zeros(self.output_shapes[idx])
		...
		def train_generator(self, input_cursors, output_cursors, train_keys): 
			while 1:
				random.shuffle(train_keys)
				lmdb_index = 0
				for idx in xrange(self.nb_train_batches):
					inputs = []
					outputs = []
					...
					for input_cursor in input_cursors:
						...
						while n > 0: 
							try:
									key = train_keys[lmdb_index]
									raw_datum = input_cursor.get(str(key))
									datum = Datum()
									datum.ParseFromString(raw_datum)
							except Exception as e:
									logger.error("Problem reading training datum with key: {} with index: {}".format(key, lmdb_index))
							...
							if datum.imgdata.data:
									...
									logger.debug("Normalized input image between 0 and 1.")
					...
					# go back to what the lmdb_index was
					lmdb_index -= self.batch_size
					...
					for output_cursor in output_cursors:
						...
						while n > 0:
							try:
									key = train_keys[lmdb_index]
									raw_datum = output_cursor.get(str(key))
									datum = Datum()
									datum.ParseFromString(raw_datum)
							except Exception as e:
									logger.error("Problem reading training output with key: {} with index: {}".format(key, lmdb_index))
									logger.error("Error: " + repr(e))
									sys.exit(-1)
						...
						oneHot_count_1 = np.zeros((self.batch_size, 5), dtype='int')
						count_1 = batch_outputs_1.sum(axis=1)
						oneHot_count_1[tuple(range(self.batch_size)), tuple(count_1)] = 1

						batch_outputs_2 = np.asarray(batch_outputs_2, dtype='int')
						oneHot_count_2 = np.zeros((self.batch_size, 8), dtype='int')
						count_2 = batch_outputs_2.sum(axis=1)
						oneHot_count_2[tuple(range(self.batch_size)), tuple(count_2)] = 1

						batch_outputs_3 = np.asarray(batch_outputs_3, dtype='int')
						oneHot_count_3 = np.zeros((self.batch_size, 7), dtype='int')
						count_3 = batch_outputs_3.sum(axis=1)
						oneHot_count_3[tuple(range(self.batch_size)), tuple(count_3)] = 1
		...
		def validation_generator(self, input_cursors, output_cursors, validation_keys):    
			while 1:
				random.shuffle(validation_keys)
				lmdb_index = 0
				for idx in xrange(self.nb_validation_batches):
					inputs = []
					outputs = []
					...
					for input_cursor in input_cursors:
						...
						while n > 0: 
							try:
									key = validation_keys[lmdb_index]
									raw_datum = input_cursor.get(str(key))
									datum = Datum()
									datum.ParseFromString(raw_datum)
							except Exception as e:
									logger.error("Problem reading validation datum with key {} for index {}".format(key, lmdb_index))
									logger.error("Error: " + repr(e))
					...
					for output_cursor in output_cursors:
						...
						while n > 0:
							try:
									...
									logger.error("Problem reading validation output with key {} for index {}".format(key, lmdb_index))
									logger.error("Error: " + repr(e))
